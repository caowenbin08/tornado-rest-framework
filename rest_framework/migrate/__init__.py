...
FILE_PATTERN = re.compile(r"[\d]{3}_[^\.]+\.py$")
...
def up(migration):
    ...
    migration({up_columns}
    )

def down(migration):
    {down_tables}

    migration({down_columns}
    )

'''

COLUMN_DEFINITION = {
    'add_column': "\n      migration.add_column('{}', '{}', {}_{})",
...
class Generator(object):
    ...
    @staticmethod
    def load_migrations_files():
        if not os.path.exists(DEFAULT_MIGRATE_DIR):
            os.makedirs(DEFAULT_MIGRATE_DIR)
        return sorted(f[:-3] for f in os.listdir(DEFAULT_MIGRATE_DIR) if FILE_PATTERN.match(f))
    ...
    def get_tables(module):
        ...
        return {m._meta.db_table: m for m in modules}
    ...
    def run(self):
        print("----self.py_tables--", self.py_tables)
        for table_name, py_table in self.py_tables.items():
            print("====py_table==", py_table)
            ...
            if table_name in self.db_tables:
                ...
                if deleted:
                    ...
                    for column in deleted:
                        self.generate_definition('drop_column', self.db_tables.get(table_name), self.db_models,
                                                 table_name, column)
        ...
        migrations_files = self.load_migrations_files()
    ...
    #         num = len(self.todo)
        ...
        name = str(uuid.uuid1())
        filename = name + '.py'
        path = os.path.join(DEFAULT_MIGRATE_DIR, filename)
