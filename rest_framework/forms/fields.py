...
from rest_framework.forms import validators
...
__all__ = [
    ...
    'IntegerField', 'DecimalField', 'FloatField', 'DateTimeField', 'DateField', 'TimeField',  'ChoiceField',
...
NOT_REQUIRED_DEFAULT = _("May not set both `required` and `default`")
...
class Field(object):
    ...
    def __init__(self, verbose_name=None, required=True, default=None, source=None, error_messages=None,
                 validators=(), null=False, read_only=False,):
        """
    ...
    def get_default(self):
        ...
        if value is None:
            default_method_name = 'get_{field_name}'.format(field_name=self.field_name)
    ...
    def to_python(self, data):
        raise NotImplementedError('`to_python()` must be implemented.')
    ...
    def get_attribute(self, instance):
        ...
        for attr in self.source_attrs:
            ...
            if functional.is_simple_callable(instance):
                instance = instance()
    ...
    def __deepcopy__(self, memo):
        print("---self._kwargs--self._args----", self._args, self._kwargs, type(self._kwargs.items()))
        args = [
        ...
        kwargs = {
            key: (copy.deepcopy(list(value) if isinstance(value, Iterator) else value) if (key not in ('validators', 'regex')) else value)
            for key, value in self._kwargs.items()
        }
...
class URLField(CharField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators.append(validators.URLValidator(message=self.error_messages['invalid']))
...
class IntegerField(Field):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.min_value is not None:
            ...
            self.validators.append(validators.MinValueValidator(self.min_value, message=message))

        if self.max_value is not None:
            message = self.error_messages['max_value'].format(max_value=self.max_value)
            self.validators.append(validators.MaxValueValidator(self.max_value, message=message))
...
class MultipleChoiceField(ChoiceField):
    ...
    def to_python(self, data):
        ...
        return {
            super(MultipleChoiceField, self).to_python(item)
            for item in data
        }
